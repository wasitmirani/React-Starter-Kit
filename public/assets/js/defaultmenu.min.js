...
(() => {
  let html = document.querySelector("html");
  const headerToggleBtns = []; 
      // headerToggleBtn = document.querySelectorAll(".sidemenu-toggle");
    const mobileBtn = document.getElementById("headerToggleBtn");
    const desktopBtn = document.getElementById("menuToggleBtn");

    // Push them to an array if they exist
    if (mobileBtn){
      headerToggleBtns.push(mobileBtn);
    } 
    if (desktopBtn){
      headerToggleBtns.push(desktopBtn);
    } 
  ...
  if (
    !localStorage.getItem("nilovalayout") &&
    !localStorage.getItem("nilovanavstyles") &&
    !localStorage.getItem("nilovaverticalstyles") &&
      ...
      switch (verticalStyle) {
        ...
        case "overlay":
          ...
          if (html.getAttribute("data-toggled") === "icon-overlay-close") {
            ...
            if (window.innerWidth >= 992) {
              if (!localStorage.getItem("nilovalayout")) {
...
function doubletFn() {
  ...
  menuSlideItem?.forEach((e) => {
    ...
    e.addEventListener("mouseenter", () => {
      if (window.innerWidth >= 992) {
        if (localStorage.nilovaverticalstyles == "doublemenu") {
...
function doublemenu() {
  if (window.innerWidth >= 992) {
    ...
    sidemenulink.forEach((ele) => {
      // Skip dark/light theme buttons
      if (!ele.hasAttribute("data-hs-theme-click-value")) {
        ele.addEventListener("click", doubleClickFn);
      }
    });
  }
